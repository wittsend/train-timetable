# Modified version of RttApi.parser to fix problem with filter key
...
def parse_search(json: dict) -> SearchResult:
    ...
    if filter_json is not None:
        out.filter = parse_filter(filter_json)
...
def parse_filter(json: dict) -> LocationDetail:
    """
    ...
    """
    if not all(key in json for key in ('destination',)):
        raise ValueError("JSON object missing required keys")

    return parse_location_detail(json['destination'])
